...
// 插件目录
define('ADDON_PATH', App::getInstance()->getRootPath() . 'addons' . DS);

// 定义路由
Route::any('addons/:addon/[:controller]/[:action]', "\\Sunny\\addons\\Route@execute");
...
Hook::add('app_init', function () {
    ...
    $rules = [];
    // 路由的命名空间
    $execute = "\\Sunny\\addons\\Route@execute?addon=%s&controller=%s&action=%s";
    foreach ($routeArr as $k => $v) {
        if (is_array($v)) {
            ...
            $route_value = sprintf($execute, $addon, $controller, $action);
            $rules[$k] = $route_value;
            Route::rule($k, $route_value);
        }
    }
    //Route::rule($rules);
    if ($domains) {
    ...
    // 获取系统配置
    $hooks = App::getInstance()->isDebug() ? [] : Cache::get('hooks', []);
    if (empty($hooks)) {
...
function get_addon_autoload_config($truncate = false)
{
    ...
    $route = [];
    // 读取插件目录及钩子列表
    $base = get_class_methods("\\Sunny\\Addons");
...
function addon_url($url, $vars = [], $suffix = true, $domain = false)
{
    ...
    $dispatch = think\facade\Request::instance()->dispatch()->getDispatch();
